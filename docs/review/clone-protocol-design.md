...
4. **`create()` が成功して `open()` が失敗した場合の補償**(第 1 巡 R1-3 で追加)。
   ...
   - **`IProject.delete(deleteContent = false, force = true, monitor)` で
     ...
     **所有権は明確**(§1.1 の問題とは異なる)。
   - **この補償自体が失敗した場合**は、その状態を明示して通知する:
     「ワークスペースに閉じたプロジェクト `<名前>` が残っています。プロジェクトエクスプローラから
     削除してください。**中身は消えません**」。**ディスク上の clone には触れない。**
...
| 解除も失敗 | 閉じた `IProject` が残る | **通知して手順を案内**。ディスクには触れない |
...
- **既存 `GitOperationGuard` をそのまま流用できる。** 同クラスの `withRepo(key: String)` は
  ...
  **正規化した clone 先の絶対パスをキーにし、clone の開始から完了(取り込みの前)まで保持する。**
  専用の guard を新設しない。
