...
改訂: round 4(Codex round 1〜3 の指摘を反映。経緯は §25)
...
| R-4 | **設定更新と競合していない場合**、ゲート不成立時の戻り値・監査ログ・ユーザー通知が、呼び出し元から見て現状と同一であること。競合時に許容する差は §9.5 に列挙する(それ以外の差は認めない) |
...
**逆方向(現行が不成立で本設計が成立)は存在しない。** 本設計が成立を返すのは述語が受理し、かつ `g1 == g2` が成立した場合のみであり、これは現行が成立を返す条件の部分集合である(R-3a)。
...
| AC-3b | 競合時に生じる差が §9.5 の一覧に収まり、それ以外の差が無いこと。とくに**成立側に緩む差が存在しない**こと | 単体テスト(20.2 a)+ §9.5 の論証 |
...
**round 4 で新たに導入し、まだレビューを受けていない論点**:

- **§9.5(競合時に許容する差の完全な一覧)**。2 件に限定できているか、他に差が生じる競合が無いか。とくに「逆方向(現行が不成立で本設計が成立)は存在しない」という主張の論証。
- **§18 の監査理由の差**。障害調査上、`connection-unstable` と `instance-mismatch` が競合時に入れ替わりうることを受容してよいか。
- **§20.2(b2) の層分け**が、モック層で固定できることと実体層でしか固定できないことを正しく切り分けられているか。

**round 3 までの論点で、まだ明示的な評価を受けていないもの**:
