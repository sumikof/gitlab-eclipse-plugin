...
[GitLabSidebarView toolbar]
  └ command com.gitlab.eclipse.commands.CreatePipeline
       └ CreatePipelineHandler (AbstractHandler / thin SWT shell)         ★新規
            ...
            ├ PipelineActionService.create(conn, projectId, ref)           ★新規(+1メソッド)
            │    └ apiClient.post("/projects/{id}/pipeline", {ref}, conn)
            └ classifyWrite / writeAuditMessage / 成功refresh (再利用)
```
...
```kotlin
fun create(connection: ConnectionSnapshot, projectId: Long, ref: String): PostResult =
  apiClient.post("/projects/$projectId/pipeline", query = mapOf("ref" to ref), connection = connection)
```
- 純粋なパス組立 + delegation（既存 retry/cancel と同型）。例外は呼び出し元へ伝播。
...
- **ノードを前提としない**ため PR-2 の `launchCiWrite`（`WriteKey` + node fingerprint 照合）はそのまま使わず、create 専用の小 launcher を同パッケージに置く。共有部（`classifyWrite` / `writeAuditMessage` / `InFlightWriteGuard` / 成功時 refresh ヘルパ `findSidebarViewIn`）は再利用できる形に抽出する（`findSidebarViewIn` を internal 化するなど、既存 PR-2 の挙動は不変）。
...
UI:  ref==null → "Cannot create a pipeline for a detached HEAD." 通知・終了
     ref!=null → 確認ダイアログ（ref, webUrl 提示）
                  Cancel → 終了
UI:  key = WriteKey(normalizeInstanceUrl(context.instanceUrl), "createPipeline", 合成ID)
...
### in-flight キーの合成 ID
`WriteKey(instanceUrl, targetKind, targetId: Long)` は targetId が Long。create の対象は (projectId, ref) の組で、ref は文字列。安定な Long を得るため **`targetId = 31 * projectId.hashCode() + ref.hashCode()` を Long 化した合成キー**とする（衝突は「異なる (project,ref) が同一 in-flight とみなされ 2 個目が無視される」= 安全側に倒れるのみ・生成漏れはユーザー再クリックで解消）。あるいは `WriteKey` を汎用化して文字列 targetId を許すか、create 専用の別キー型を用意する。**未決事項 O-1** 参照。
...
| detached HEAD（ref null） | 「Cannot create a pipeline for a detached HEAD.」通知・中止 |
...
- in-flight ガードは (instance, "createPipeline", 合成ID) で同一 create の同時実行を直列化。
- 異なる (project, ref) の create は並行可能。
...
- 成功/失敗とも `writeAuditMessage`（token 非出力）で 1 行構造化ログを Error Log に出力（action="create", instanceUrl, projectId, targetKind="pipeline", targetId=合成ID or projectId, outcome）。
- 連打で無視した場合は info ログ。
- 監査ログの targetId 表現は §9 の合成 ID に依存（未決 O-1）。
...
- headless TDD（sonnet）: `PipelineActionService.create` のパス/query 組立、`sameConfiguredInstance` の正規化比較（末尾スラッシュ差・大文字小文字は既存 `normalizeInstanceUrl` に準拠）。
...
- AC-4: 連打しても in-flight 中の 2 回目は無視される（単一 POST）。
- AC-5: 設定済み接続がリポジトリのインスタンスと異なる場合、POST されず通知される。
- AC-6: 失敗時に token を含まない監査ログが 1 行残り、汎用失敗通知が出る。
- AC-7: `./gradlew build` が detekt 0・対象テスト PASS・ベースライン外の新規失敗ゼロ。

## 25. 未決事項

- **O-1（in-flight キー / 監査 targetId の表現）**: create の対象は (projectId, ref) の組で、既存 `WriteKey.targetId: Long` と ref: String がミスマッチ。(a) 合成 Long ハッシュ、(b) `WriteKey` 汎用化で文字列許容、(c) create 専用キー型、のいずれか。衝突時の挙動（安全側=2 個目無視）と監査ログの可読性（targetId に ref を出せるか）のトレードオフ。実装計画で確定する。
