   ...
   - **clone(R2-7 → 第 3 巡 R3-7 で方式変更)**: 第 2 巡では「コマンドが作成した子要素だけを削除」
     ...
     **Linux では既存の空ディレクトリを置換できる**。したがって「事前に存在確認 → 転送 → move」では、
     ...
     - **clone は宛先直下の専用ステージング用サブディレクトリへ行う**(第 6 巡 R6-1)。
       ...
       > **⚠️ 未解決(第 7 巡 R7-1・P1)**: 当初ここで「ステージング用サブディレクトリの名前は
       > 本コマンドしか知らないため配下は全て自分の生成物」と述べていたが、**これは成立しない**。
       > ディレクトリは作成された時点で通常の列挙やファイル監視から発見でき、
       > 同期ツールはその配下にも書き込める。**名前の秘匿は所有権の根拠にならない。**
       >
       > **この論点(clone の所有権・確定・回復プロトコル)は、Codex の評価でも
       > 「独立した実装サイクルに切り出すのが妥当」とされた。**
       > 関連して未解決のまま残る指摘: **R7-2**(ステージング昇格時の同名衝突と部分昇格)、
       > **R7-3**(`createDirectory` と `CloneReservation` 永続化の間のクラッシュ窓)。
       > **切り出しの可否はユーザー判断待ち。** 判断が出るまで **F5 / F7 は実装着手不可**とする。
       > 他の 6 コマンド(F1〜F4 / F6 / F8)はこの論点に依存しないため影響を受けない。
