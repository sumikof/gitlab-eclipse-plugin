...
| 項目 | 規定 |
|---|---|
| 最低保持期間 | この期間内の退避は**上限に達しても削除しない**。上限に達した場合は**新しい退避を作らず適用を中止**する(古いものを消して進むより安全側) |
...
> **A18(訂正版)**: 適用によって上書きされる各ファイルについて、
> **最終照合時点の内容が退避領域に保存されていること**。
> **最終照合から rename までの窓(システムコール 1 回分)に書き込まれた内容は捕捉できない** —
...
3. **[UI]** クローン先ディレクトリを選択。**既に存在すれば中止**(第 4 巡 R4-3)。
   宛先は成功時の atomic rename で**初めて生成される**。既存の空ディレクトリを選ばれた場合は、
   その配下の新規サブディレクトリを提案するか選び直してもらう(**削除はしない**)。
   ...
   - **clone(R2-7 → 第 3 巡 R3-7 で方式変更)**: 第 2 巡では「コマンドが作成した子要素だけを削除」
     ...
     **Linux では既存の空ディレクトリを置換できる**。したがって「事前に存在確認 → 転送 → move」では、
     ...
     - **開始時に `Files.createDirectory(宛先)` で宛先を予約する。**
       ...
       成功した時点で**その宛先はこのコマンドが作ったものであることが保証される**。
     - **予約した宛先へ直接 clone する。** 一時ディレクトリも move も要らない。
     - キャンセル・失敗時は**予約した宛先を丸ごと削除する**。
       第 3 巡 R3-7 で「作成した子要素を識別できない」ことが問題になったが、
       **ディレクトリ自体を自分で作った以上、その配下は全て自分の生成物**として扱えるため、
       丸ごと削除で正しい。**元から存在したディレクトリを消す経路は存在しない**
       (存在したら予約が失敗して着手しないため)。
...
| A21 | 退避領域が、**最低保持期間・総容量/件数の上限・削除順序・起動時の清掃**の規定どおりに管理されること。上限到達時は**古い順**に削除し、**削除前に通知**する(§9.4.2 / R5-3) |
...
|---|---|---|
| clone(F5 / F7) | 宛先の予約(`Files.createDirectory`)| **予約した宛先を丸ごと削除**する。予約は既存ディレクトリでは失敗するため、**削除対象は必ずこのコマンドが作ったもの**(§14-3)。通知する |
...
| **F5** cloneWiki | `.wiki.git` に変換された URL が、**`Files.createDirectory` で予約した宛先へ直接**クローンされ、Eclipse プロジェクトとして取り込まれる | プロジェクト/URL/ディレクトリ選択で閉じても**予約を行わない**(ディレクトリを作らない) | 失敗時は**予約した宛先を丸ごと削除**する。既存ディレクトリは予約が失敗するため**削除対象になりえない**(§14-3) | **A6**: 宛先が**既に存在すれば予約が失敗し中止**。予約の成否が唯一の判定であり、事前確認と作成の間に窓を作らない |
