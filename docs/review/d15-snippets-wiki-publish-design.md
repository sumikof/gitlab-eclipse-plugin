...
5. **[背景・排他境界の内側]** **JGit に作業ツリーを触らせない。**
   ...
   3. **HEAD ツリーと適用後ツリーを diff** し、変更集合を確定する
      ...
      JGit が勝手に書いたものではない。
   4. 変更集合の各パスについて、**書き込み直前に CAS 照合**する:
      作業ツリーの現在の内容ハッシュ・修正スタンプ・モードが**期待する適用前の値**と一致するか。
      一致しなければ**外部変更**とみなし、そのパスには書かず中止に入る。
   5. 一致したパスへ**自分で書き込む**(内容 + モード。実行可能ビットを含む)。
      書いたパスと**その post-image のハッシュ**を記録していく。
   6. 途中で失敗した場合、**自分が書いたパスだけを戻す**。戻す前にも CAS 照合するが、
      比較対象は**適用前の値ではなく 5 で記録した post-image** である
      —(適用前の値と比べると自分の書き込みで必ず不一致になる。第 3 巡 R3-1 の指摘)。
      post-image と一致しないパスは**外部で変更されたものとして戻さない**。
   7. 作業ツリーへの書き込みが**全て成功した場合にのみ**、index を更新する。
      **index の更新も自分で行う**ため、JGit が index を書いた後の外部 `git add` との競合(R3-6)は
      構造的に発生しない。
...
2. **書き込みのたびに CAS 照合する。** §9.4-4/6 のとおり、書き込む直前に現在の内容ハッシュ・
...
3. **応答不明のときのみ照会**する。照会でプロジェクトが見つかった場合でも自動採用はせず、
   **所有関係を確認できるときのみ採用**する(作成者が現在のユーザーであること、
   および作成時刻が intent の記録時刻の近傍にあること)。
...
```
PublishRecord =
  | Intent(repositoryRootPath, instanceUrl, namespacePath, projectPath)
...
3. **後始末を定義する**: キャンセル/タイムアウト時、
   - **clone(R2-7 → 第 3 巡 R3-7 で方式変更)**: 第 2 巡では「コマンドが作成した子要素だけを削除」
     ...
     **識別に頼るのをやめ、隔離する。**
     - **常に新規の一時ディレクトリへ clone し、成功したときだけ最終的な宛先へ移動する。**
     - キャンセル・失敗時の後始末は**一時ディレクトリを丸ごと削除するだけ**。
       ユーザーが指定した宛先は、成功して移動が完了するまで**一切触らない**。
     - 一時ディレクトリは**最終宛先と同一ファイルシステム上**に作る(移動を atomic な rename にするため)。
     - 移動先が空でないことが移動直前に判明した場合は中止し、一時ディレクトリを削除する
       (§9.5 の事前チェックとの間の窓を閉じる)。
...
|---|---|---|
| clone(F5 / F7) | 一時ディレクトリへの転送開始 | **一時ディレクトリを丸ごと削除**する。ユーザー指定の宛先は移動完了まで触れていないため**無変化**(§14-3)。通知する |
...
| **F5** cloneWiki | `.wiki.git` に変換された URL が**一時ディレクトリへ**クローンされ、成功時のみ宛先へ移動されて Eclipse プロジェクトとして取り込まれる | プロジェクト/URL/ディレクトリ選択で閉じてもディレクトリを作らない | 失敗時は**一時ディレクトリのみ削除**し、指定された宛先は**無変化**(§14-3) | **A6**: 空でないディレクトリなら中止(移動直前にも再確認) |
