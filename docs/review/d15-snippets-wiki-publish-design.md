...
| **バイナリ変更を含むパッチ**(第 4 巡 R4-4) | 同梱 JGit 7.5 の `DiffCommand` / `DiffFormatter` は、バイナリ変更について **`Binary files differ` しか出力せず内容(literal/delta payload)を持たない**。そのまま F3 でスニペット化しても **F4 で post-image を再現できない**。したがって **F3 はバイナリ変更を検出したら作成を拒否**し、対象ファイルがある旨を通知する(件数のみ・パスは出さない)。F4 側も、パッチにバイナリハンクが含まれる場合は適用しない。Git binary patch の生成対応は別サイクル |
...
2. **可復元性(こちらが本質)**: **上書きする前の内容を必ず退避領域へ保存する。**
   ...
   万一 1 の窓で外部の保存を上書きしても、**その内容は退避領域から復元できる**。
3. **告知**: 適用が中止・復元された場合、および退避を行った場合は、
   **退避領域の存在と対象件数を通知する**(パスは出さない — §16)。
   退避領域は成功後も一定期間残し、古いものから削除する。

**受け入れ条件 A18**: 適用中に外部プロセスがファイルを保存した場合でも、
**その内容が退避領域から復元できる**こと。「上書きが絶対に起きない」ことは要求しない
(実現できないため)。**「失われない」ことを要求する。**
...
3. **後始末を定義する**: キャンセル/タイムアウト時、
   - **clone(R2-7 → 第 3 巡 R3-7 で方式変更)**: 第 2 巡では「コマンドが作成した子要素だけを削除」
     ...
     **識別に頼るのをやめ、隔離する。**
     ...
     - 一時ディレクトリは**宛先の親ディレクトリ配下**に作る(移動を atomic な rename にするため
       同一ファイルシステム上に置く)。
     - **宛先は「存在しないこと」を要件とする(第 4 巡 R4-3)。**
       既存ディレクトリへの rename は通常失敗し、`REPLACE_EXISTING` に頼れば
       **ユーザー所有のディレクトリごと置き換える**ことになる。したがって §9.5 の
       「空でなければ中止」を**「存在すれば中止」に変更**する。
       宛先ディレクトリは**この rename によって初めて生成される**。
     - ユーザーが既存の空ディレクトリを選んだ場合は、**その配下の新規サブディレクトリを提案**するか、
       別の場所を選び直してもらう(削除はしない)。
     - rename が失敗した場合(競合により宛先が先に作られた等)は中止し、一時ディレクトリを削除する。
...
| A18 | パッチ適用中に**外部プロセス**が対象ファイルを保存した場合でも、その内容が**退避領域から復元できる**こと(§9.4.1)。「上書きが絶対に起きない」ことは要求しない — 外部プロセスに対する真の CAS は実現できないため、**失われないこと**を要求する(R4-1) |
...
| **F5** cloneWiki | `.wiki.git` に変換された URL が**一時ディレクトリへ**クローンされ、成功時のみ宛先へ移動されて Eclipse プロジェクトとして取り込まれる | プロジェクト/URL/ディレクトリ選択で閉じてもディレクトリを作らない | 失敗時は**一時ディレクトリのみ削除**し、指定された宛先は**無変化**(§14-3) | **A6**: 宛先が**既に存在すれば中止**。rename が失敗した場合も中止して一時ディレクトリを削除する |
