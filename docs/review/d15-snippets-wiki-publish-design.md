...
**排他境界について**: `GitOperationGuard` が排他するのは**プラグイン内の git 操作のみ**で、
Eclipse のエディタ編集・保存は止められない。したがって「編集を止める」のではなく
**「適用直前に変化を検出したら中止する」**という検出型の設計を採る。5-3 と 5-5 の間に残る窓は
ミリ秒オーダーであり、そこで保存が起きた場合は 5-5 の復元と 6 の refresh により
**作業ツリーは適用前の状態に戻る**(ユーザーの編集が失われない側に倒す)。
...
6. **[背景]** `POST /projects` に `{path, namespace_id, visibility}`。
7. **[背景]** **発行状態を記録する**(リポジトリ識別子 → 作成したプロジェクトの URL と remote 名)。
   これが再開の根拠になる。
8. **[背景]** remote を追加し、現在のブランチを push する。
...
**再開(R1-5)**: push だけが失敗した場合、作成済みプロジェクトと追加済み remote は**残す**(§12)。
再実行時、フェーズ 0 の「remote があれば中止」に**例外**を設ける —
**7 で記録した発行状態が存在し、かつ既存 remote がその記録と一致する場合に限り**、
**プロジェクトを作らずに push だけを再試行**する。一致しなければ従来どおり中止する。
これにより「手作業で remote を消して再実行 → プロジェクト重複作成」を防ぐ。
成功したら記録を破棄する。
...
- `PublishState(repositoryRootPath, remoteName, projectWebUrl)` — §9.6 の再開判定に使う。成功で破棄。
...
3. **後始末を定義する**: キャンセル/タイムアウト時、
   - clone: **このコマンドが作成したクローン先ディレクトリを削除**する(既存ディレクトリは触らない。
     §9.5 で「空でなければ中止」としているため、作成したものだけが対象になる)。
...
| A15 | いずれのコマンドも、**キャンセル時に副作用を残さない**(API 要求を出さない・ファイルシステムを変えない) |
...
| **F4** applySnippetPatch | パッチが作業ツリーに適用され、**`refreshLocal` 後に Eclipse から見える** | 一覧選択で閉じても作業ツリーを変更しない | **A11** の復元 | **A3**: 適用直前の再検証で dirty/変更を検出したら中止し作業ツリーを変更しない。`.patch` blob を持つスニペットが 0 件なら通知して終了 |
...
| **F8** selectProject | 割当が保存され、**以後の解決が割当を優先する**。**A7**: Eclipse 再起動後も保持される | 保存しない | — | **A13** / **A14** |
