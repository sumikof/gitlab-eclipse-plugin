...
`AuthState.UNKNOWN` は enum に残すが、公開の規則の中で「保存なし」と同じ扱いの名前として使うだけで、`update` が書くことはない。
