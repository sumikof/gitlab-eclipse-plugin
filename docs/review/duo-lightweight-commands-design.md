...
| R7 | **Code Suggestions が有効な環境では**(ローカル設定 `CODE_SUGGESTIONS_ENABLED` と `code_suggestions` feature state の両方が有効 = `CodeSuggestionsSession.kt:481-483` の `isGloballyEnabled()` と同じ条件)、開いたエディタで **Code Suggestions が動く**こと。**無効な環境でも Tutorial は作成して開き**(Chat の手順も含む学習用の文書のため。VSCode もコマンドの表示条件は Chat の可否だけで、補完の状態を見ない、`package.json:218-221`)、**エディタを開いた時点で状態を読み直し**、無効なら開いた後に情報通知(`NotificationUtils.show`)を出す。文言は理由で分ける(Codex round 24 P2 反映): (i) **ローカル設定 `CODE_SUGGESTIONS_ENABLED` が false** →「Code Suggestions is turned off. Turn it on from the GitLab Duo status menu to try the completion steps.」(既存の `ToggleCodeSuggestionsHandler` で直せる); (ii) **ローカル設定は true だが `code_suggestions` の check が engaged**(`CodeSuggestionsStateService.getFirstEngagedCheck()`、`CodeSuggestionsStateService.kt:23`)→「Code Suggestions is unavailable: <check の表示名>. Open GitLab Duo diagnostics for details.」(表示名は `FeatureStateLabels.labelFor(checkId)`、`diagnostics/FeatureStateLabels.kt:56`。ローカルのトグルでは直らないので案内しない。診断は既存コマンド `gitlab-eclipse-plugin.commands.ShowDiagnostics`、`plugin.xml:327`); 両方に当たる場合は (i)(Codex round 23 P1 反映)。有効な環境での条件は、すなわち (a) `IFileEditorInput` で開かれ `didOpen` が送られる(`GitLabLanguageServerOpenFilesService.kt:119-125, 144-149`)、(b) `languageId` が `javascript` になる(`LanguageServerLanguage.kt:15, 34-40`: 拡張子 `js` → `"javascript"`)、(c) JavaScript が既定で有効である(`CodeSuggestionsLanguageService.kt:22-29`: サポート言語は `CODE_SUGGESTIONS_DISABLED_SUPPORTED_LANGUAGES` に含まれない限り有効、既定値は `""`(`PreferenceInitializer.kt:17`))。 |
...
execute(event)                                   ← UI スレッド
  ├ 0. 設定 `DUO_ENABLED_WITHOUT_GITLAB_PROJECT` が false → 確認ダイアログ(§9.2 非 GitLab プロジェクト。「Open Preferences」で設定ページを開く)を出して終了(Codex round 22 / 23 P1 反映)
  └ 1. Writer(WorkspaceJob, rule=root).schedule() だけを行う(UI スレッドでは所有も拒否も判定しない。Codex round 18 P2 反映)
     ...
     Job 完了(JobChangeAdapter.done):
         Ready → asyncExec { OpenOnUi(file) }(下記。rule は持たない)
         Refused(reason) → §9.2 の拒否の通知
         Failed → 通知「Could not create the GitLab Duo Tutorial project. See the Error Log.」
         Cancelled → 何もしない
  OpenOnUi(UI スレッドで `IProgressService.runInUI(window, runnable, root)` を呼び、root rule を**イベントループを回しながら**待って取得・解放する。Codex round 18 P2 / round 20・21 P1 反映。`runInUI` の呼び出し全体を捕捉する: `InterruptedException` → キャンセル(通知なし)、`InvocationTargetException` / その他の例外 → 失敗の通知「Could not open the GitLab Duo Tutorial. See the Error Log.」+ 例外クラス名だけのログ。Codex round 22 P2 反映)
         runInUI の runnable 内: **開く直前に所有をもう一度検証**(同名プロジェクトが開いている・persistent property が記録 ID と一致・locationURI が記録と一致・`duo_tutorial.js` が存在)
                        → 一致 → openInActiveEditor(file) → **この時点で** Code Suggestions の状態を読み直し(ローカル設定 `CODE_SUGGESTIONS_ENABLED` と `CodeSuggestionsStateService`)、無効なら R7 の情報通知(Codex round 24 P1 反映: コマンド開始時には読まない)
...
**非 GitLab プロジェクトと LS のプロジェクト方針(Codex round 22 P1 で訂正)**: 以前の版の「`enabledWithoutGitlabProject == false` でも Tutorial で Duo は無効にならない」は**誤り**だった。LS の `duo-disabled-for-project` チェックは 2 経路ある。(a) `validate(config)`(LS map @28563357)は、設定が `true` なら非 engaged、`false` なら `DuoDisabled` の workspace フォルダがあるときだけ engaged。(b) 文書単位の `#checkIfProjectHasDuoAccess`(@28562062)は、`#hasDuoAccess` を**設定値で初期化**し、プロジェクトの状態が `DuoEnabled` / `DuoDisabled` のときだけ上書きする。Tutorial は GitLab プロジェクトではない(`DuoProjectStatus.NonGitlabProject`、@28416839)ので上書きされず、**設定が `false` なら `#hasDuoAccess == false` のまま engaged になり、Duo(Code Suggestions を含む)が無効になる**。

**したがって F2 は実行時にこの設定を確かめる**: Writer を起動する前(ハンドラ、UI スレッドで設定ストアを読むだけ。ワークスペースは読まない)に `PreferenceConstants.DUO_ENABLED_WITHOUT_GITLAB_PROJECT`(`PreferenceConstants.kt:21`、既定 `true` = `PreferenceInitializer.kt:22`、LS へは `GitLabLanguageServerConfigurationService.kt:154-156` で送信)を読み、`false` なら **Tutorial を作らず**、確認ダイアログ `MessageDialog.openQuestion(shell, "GitLab Duo Tutorial", <本文>)`(前例: `DeleteNoteHandler.kt:49` の `MessageDialog.openConfirm`)を出す。本文は「The GitLab Duo Tutorial needs "Enable Duo features when no GitLab project is detected" in the GitLab preferences. Open the preferences now?」(設定名は `GitLabPreferencePage.kt:216-221` のラベルと一致させる)。「Yes」なら `openGitLabPreferences()`(`PreferencesUtil.kt:5-11`)を開き、「No」なら何もしない。**`NotificationUtils.show` は使わない**(`NotificationUtils.kt:58-70` のポップアップは `Label` だけで約 1 秒で閉じ、リンクや選択の callback を持てないため導線にならない。Codex round 23 P1 反映)。ダイアログはユーザー自身がコマンドを実行した直後に出すのでモーダルでよい。ダイアログの表示と設定ページのオープンは注入シームの背後に置き、headless で「質問が 1 回出る・Yes で設定ページのシームが 1 回呼ばれる・No で呼ばれない」を試験する。メニュー項目は表示したまま(発見可能性のため。R10 の `duo_chat_enabled` 条件は不変)。設定が `true` の既定環境では従来どおり作成する。
...
| F2 | `runInUI` の rule 待機をユーザーがキャンセル(`InterruptedException`) | キャンセル扱い。通知なし。Tutorial はできているので再実行で開ける |
| F2 | `runInUI` 内の再検証・open の例外(`InvocationTargetException`) | 通知「Could not open the GitLab Duo Tutorial. See the Error Log.」+ 例外クラス名だけのログ。UI イベントへ例外を漏らさない |
| F2 | 設定「Enable Duo features when no GitLab project is detected」が無効 | Tutorial を作らず、確認ダイアログ(Yes で設定ページを開く)。ワークスペースは変更しない(§9.2) |
| F2 | Code Suggestions が無効(ローカル設定または feature state。エディタを開いた時点で判定) | Tutorial は作成して開き、開いた後に理由別の情報通知(R7: ローカル設定 → トグルの案内 / engaged な check → 理由と診断の案内) |
...
| A4 | **Code Suggestions が有効な環境で**、チュートリアルコマンドで `GitLab Duo Tutorial/duo_tutorial.js` が作られエディタで開き、`didOpen` の `languageId` が `javascript`(LS ログで確認)で、本文中の `const multiply` 直後で Code Suggestions が出る。**Code Suggestions が無効な環境では**、Tutorial は作成されて開き、開いた後に R7 の情報通知が出る | 実機(M3 / M3c)+ 自動(`DuoTutorialHandlerTest`) |
...
| A7 | 設定「Enable Duo features when no GitLab project is detected」が有効(既定)の環境で、チュートリアル実行後も Duo Chat / Code Suggestions が `duo-disabled-for-project` にならない。**設定が無効の環境では Tutorial を作らず、理由と設定への導線を通知する**(ワークスペースは変更しない) | 実機(M3 で Diagnostics を確認、M3b)+ 自動(`DuoTutorialHandlerTest`) |
...
| M3b | 設定で「Enable Duo features when no GitLab project is detected」を無効にして保存 → 「GitLab Duo Tutorial」をクリック | Tutorial は作られず、設定を有効にするか尋ねる確認ダイアログが出る。「Yes」で GitLab の設定ページが開く。「No」では何も起きない。有効に戻して再実行すると作成される |
| M3c | ステータスメニューで Code Suggestions を無効にする → 「GitLab Duo Tutorial」をクリック | Tutorial は作成されて開き、Code Suggestions が無効である旨の情報通知が出る |
...
| 22 | P1 非 GitLab プロジェクト設定を実行条件に含めよ | 採用(以前の記述の誤りを訂正)。LS の `#checkIfProjectHasDuoAccess`(LS map @28562062)は `#hasDuoAccess` を設定値で初期化し `DuoEnabled` / `DuoDisabled` でしか上書きしないため、設定が無効だと非 GitLab の Tutorial で Duo が無効になる。実行時に設定を確かめ、無効なら作らずに理由と設定への導線を通知。A7 を条件付きにし M3b を追加 | §9.2, §9.2 フロー, §14, A7, M3b, `DuoTutorialHandlerTest` |
| 22 | P2 runInUI の失敗とキャンセルを完了結果へ変換せよ | 採用。`runInUI` 全体を捕捉し、`InterruptedException` はキャンセル(通知なし)、`InvocationTargetException` などは失敗の通知 + クラス名だけのログ | §9.2, §14, `DuoTutorialHandlerTest` |
| 23 | P1 Code Suggestions の無効設定も実行条件に含めよ | 採用(条件付き要件へ)。R7 / A4 を「Code Suggestions が有効な環境で」に限定し、無効な環境(ローカル設定 / feature state)でも Tutorial は作成して開き、開いた後に情報通知で有効化を案内(Chat の手順も含む学習文書のため。VSCode もコマンドの表示条件に補完の状態を含めない)。M3c と試験を追加 | R7, A4, §9.2 フロー, §14, M3c, `DuoTutorialHandlerTest` |
| 23 | P1 設定への導線を実装可能な通知契約として定義せよ | 採用。`NotificationUtils.show`(`Label` のみ・約 1 秒で閉じる)ではなく `MessageDialog.openQuestion` の確認ダイアログにし、「Yes」で `openGitLabPreferences()` を開く。表示と設定ページのオープンは注入シームで試験 | §9.2, §14, M3b, `DuoTutorialHandlerTest` |
| 24 | P1 Code Suggestions 状態は open 時に再評価せよ | 採用。コマンド開始時の判定(`csEnabled`)を削除し、所有の再検証とエディタのオープンの後、UI スレッド上でローカル設定と feature state を読み直して案内を決める。開始後に状態を切り替える試験を追加 | §9.2 フロー, R7, §14, `DuoTutorialHandlerTest` |
| 24 | P2 feature-state 無効時には実行可能な復旧案を示せ | 採用。ローカル設定が無効ならトグルの案内、ローカル設定は有効で `code_suggestions` の check が engaged なら `FeatureStateLabels.labelFor` の表示名と診断(`ShowDiagnostics`)の案内に分け、文言を試験 | R7, §14, `DuoTutorialHandlerTest` |
