...
execute(event)                                   ← UI スレッド
  ├ 0. 設定 `DUO_ENABLED_WITHOUT_GITLAB_PROJECT` が false → 通知(§9.2 非 GitLab プロジェクト)して終了(Codex round 22 P1 反映)
...
**したがって F2 は実行時にこの設定を確かめる**: Writer を起動する前(ハンドラ、UI スレッドで設定ストアを読むだけ。ワークスペースは読まない)に `PreferenceConstants.DUO_ENABLED_WITHOUT_GITLAB_PROJECT`(`PreferenceConstants.kt:21`、既定 `true` = `PreferenceInitializer.kt:22`、LS へは `GitLabLanguageServerConfigurationService.kt:154-156` で送信)を読み、`false` なら **Tutorial を作らず**、通知「The GitLab Duo Tutorial needs "Enable Duo features when no GitLab project is detected" (GitLab preferences). Enable it and run the command again.」(文言は設定ページのラベル `GitLabPreferencePage.kt:216-221` と一致させる)を出し、通知から `openGitLabPreferences()`(`PreferencesUtil.kt:5-11`)を開けるようにする。メニュー項目は表示したまま(発見可能性のため。R10 の `duo_chat_enabled` 条件は不変)。設定が `true` の既定環境では従来どおり作成する。
...
| F2 | 設定「Enable Duo features when no GitLab project is detected」が無効 | Tutorial を作らず、理由と設定への導線を通知(§9.2)。ワークスペースは変更しない |
...
| M3b | 設定で「Enable Duo features when no GitLab project is detected」を無効にして保存 → 「GitLab Duo Tutorial」をクリック | Tutorial は作られず、設定を有効にするよう促す通知が出る。通知から設定ページが開ける。有効に戻して再実行すると作成される |
