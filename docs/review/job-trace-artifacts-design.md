...
- trace 内容はメモリ上の `JobLogContent` に保持し、**contentRegistry は弱参照値**のため全エディタ入力が消えれば GC される(Vl8Ym)。ディスクに書かないため権限/symlink/cleanup/同一プロセス再起動のファイル競合が構造的に無い。
