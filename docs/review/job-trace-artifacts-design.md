...
- **可変コンテンツ参照(key ごとに共有・弱参照レジストリ)**: `JobLogEditorInput` は**可変の `JobLogContent`**(現在の text を保持)への参照を持ち、`getStorage()` は**その時点の text** から `JobLogStorage` を生成する。`equals/hashCode` は `JobLogKey` のみに依存(text を含めない=同一ジョブは同一入力)。**`JobLogContent` は `JobLogKey` ごとに 1 インスタンスを共有**するが、レジストリは **`Map<JobLogKey, WeakReference<JobLogContent>>`(UI スレッド専有・弱参照値)**とする。
- **エディタ履歴への残留を防ぐ + 明示解放(Vl8Ym/Vl96H)**: `exists()==true` の入力は Eclipse `EditorHistory` が閉じた後も最大 100 件を**強参照**するため、弱値 Map でも入力経由で content が生存し GC されない。よって:
  - **`JobLogEditorInput.exists()` は `false`**(transient 扱い=`EditorHistory`/「最近使用したファイル」に保持されない)とする。
  - さらに **最後の実エディタが閉じられたイベントで**(`IPartListener2.partClosed` を 1 度だけ登録し、同 `JobLogKey` のエディタが他ページ含め 0 になったら)**`content.text=""` に消去し `contentRegistry[key]` を除去**する。これで履歴やその他の残参照があっても大容量 trace(機密)を確実に解放する。
  - 上記により `JobLogEditorInput` が content を強参照しても、全エディタ close で content は GC 可能かつ text は即時消去され、§7.1/§14.3 の「閉じれば解放」契約を満たす。次回オープンは新しい content を生成する。
- `JobLogEditorOpener.openOrReload(key, text)`:
  - **共有 content を取得/生成**: `contentRegistry[key]?.get()` が生存していればそれを、無ければ新規 `JobLogContent` を生成し弱参照で登録。`content.text = text` に更新。以降の全エディタは同一 content を参照する。
...
// contentRegistry: Map<JobLogKey, WeakReference<JobLogContent>>(UI スレッド専有・弱参照値)
//   入力(JobLogEditorInput)が content を強参照 → 全エディタ close で GC 可能(Vl8Ym)
...
class JobLogEditorInput(val key: JobLogKey, val content: JobLogContent) : IStorageEditorInput {
  ...
  override fun exists() = false                                 // transient: EditorHistory に保持させない(Vl96H)
  override fun equals(o) = o is JobLogEditorInput && o.key == key
  override fun hashCode() = key.hashCode()
}
// 最後の同 key エディタ close で content.text="" + registry 除去(IPartListener2.partClosed・Vl96H)
...
- trace 内容はメモリ上の `JobLogContent` に保持。**contentRegistry は弱参照値**+ **`exists()==false`(EditorHistory 非保持)**+ **最後の close で `text=""` 明示解放**により、全エディタ close で確実に解放・GC される(Vl8Ym/Vl96H)。ディスクに書かないため権限/symlink/cleanup/同一プロセス再起動のファイル競合が構造的に無い。
...
- **単体(headless で検証可)**:
  ...
  - **複数ページ更新+可視化+共有 content(Vl21v/Vl4bf/Vl6vN)**: 複数 window/page に同一 `JobLogKey` のエディタが開いている状況をモックし、`openOrReload` が**全ページの一致エディタ**を更新する(いずれのページにも古い trace が残らない)こと、**アクティブページに一致が無い場合はアクティブページで新規オープン**して可視化されること、**同一 key の全 `JobLogEditorInput` が共有 `JobLogContent` を参照**し、他ページ入力を後日 provider 再接続しても最新 text を返すこと(独立 content で古 text に戻らない=Vl6vN)、**`exists()==false`** であること、および **最後の同 key エディタ close(partClosed)で `content.text` が消去され `contentRegistry[key]` が除去される**こと(履歴に入力が残っても大容量 trace が解放される=Vl8Ym/Vl96H)を検証。
...
- **U5(実装時タスク・Codex #39 Vl96H/Vl_5L/Vl_5M)**: trace メモリの明示解放リスナーのライフサイクル。`exists()==false`(EditorHistory 非保持)+ 弱参照レジストリを基本とし、加えて「最後の同 key エディタ close で `content.text` 消去+registry 除去」を行う場合、`IPartListener2` は **全 workbench window の part service に登録**する必要がある(`IWindowListener` で既存+新規 window をカバー)+ **`GitLabEclipseStartup.stop` で全 window/page listener を `removePartListener`・`removeWindowListener` して旧 listener/旧 bundle classloader の蓄積を防ぐ**。設計レビューはここで打ち切り(ユーザー判断)、本ライフサイクルは**実装時に上記契約で実装し、実装 PR の @codex ラウンドで検証**する。基本の「exists()==false + 弱参照 + partClosed 解放」自体は §7.2 に記載済み。
