...
- **改訂履歴**:
  ...
  - rev8 = Codex round 6(P1×1)反映(一部プッシュバック)。SSH バンドルの供給機序を正確化: jgit core は feature.xml 非掲載で fat-bundle 同梱のため、SSH も同機序(feature.xml 追加ではない)。Tycho 出荷 update-site への供給・解決性を事前確認事項 U-10 + R-7 として明記(§7.1/§17/§21/§24)。
...
- **SSH transport(必須依存の追加・明示)**: JGit core だけでは SSH 通信を開始できない(`SshSessionFactory` 実装が無いと fetch/push が実行時 transport error)。したがって SSH remote を扱うには **`org.eclipse.jgit.ssh.apache`(Apache MINA sshd ベース)を必須依存として追加**する。**プロセス全体の静的置換(`SshSessionFactory.setInstance(...)`)はしない**(同一 JVM の EGit や他プラグインの factory を奪い、認証・proxy・host-key 処理を壊すため)。代わりに、当該 `FetchCommand`/`PushCommand` の **`setTransportConfigCallback` から対象 `SshTransport` にだけ** プラグイン所有の `SshdSessionFactory` を設定し、ライフサイクルもプラグイン内に閉じる。鍵/エージェント/known_hosts はユーザーの `~/.ssh`(および ssh-agent)を用いる。self-managed 環境の host-key 検証(未知ホストの扱い)は実機検証項目(§21 R-3)。
  - **依存追加の扱い(CLAUDE.md 準拠・供給機序を正確化)**: 本プロジェクトのサードパーティ実行時依存(jgit core・guava・coroutines・koin・scribejava 等)は **`feature/feature.xml` には列挙されず**、`build.gradle.kts` の `implementation` 依存として **`configurations.runtimeClasspath` を zipTree でプラグインバンドル jar に同梱**する fat-bundle 方式(build.gradle.kts:196-218)で供給されている(jgit core もこの方式)。したがって SSH バンドルも **jgit core と同一機序(バンドル jar 同梱の Maven `implementation`)で供給する**のが原則で、**feature.xml への追加ではない**。PR で依存追加を明示(§17)。
  - **出荷 update-site(Tycho)への供給 = 事前確認事項 U-10**: 本リポジトリは Gradle(fat-bundle/equoIde)に加え **Tycho reactor(root/`feature`/`update-site`/`bundles` の pom.xml)**でも出荷 update-site を組み立てる。SSH バンドル + **Apache MINA sshd の推移依存**(sshd-core/osgi、agent connector とそのランタイム。JNA 等を伴う場合あり)が、**jgit core が現在出荷バンドルへ供給されているのと同じ経路で解決・同梱される**ことを PR-3 着手前に確認する。既存 p2/Maven ソースで JGit 7.5 の ssh バンドルが解決できない場合、p2 リポジトリ追加は**ビルド構成変更に当たり得るためユーザーの明示承認が前提**(§21 R-7)。
  - **ssh-agent 認証の依存(JGit 7.5)**: 鍵を `ssh-agent` のみに登録した環境では `org.eclipse.jgit.ssh.apache` 単体では agent connector が無く agent 認証が動かない。**`org.eclipse.jgit.ssh.apache.agent` とそのランタイム依存**も上記機序で同梱し、公開鍵ファイル + agent 双方をカバーする。
...
- **SSH transport 依存**(§7.1、U-8 確定=採用): `org.eclipse.jgit.ssh.apache`(+ ssh-agent 認証には `org.eclipse.jgit.ssh.apache.agent` とランタイム依存)を **jgit core と同一機序 = `build.gradle.kts` の `implementation` 依存としてバンドル jar に同梱**(feature.xml へは追加しない。jgit core も feature.xml 非掲載)。PR で明示(CLAUDE.md「必須依存の追加は PR で明示」)。**Tycho reactor 側の解決性は U-10 で事前確認**(既存 p2/Maven で JGit 7.5 ssh + MINA sshd 推移依存が解決できない場合の p2 追加はビルド構成変更に当たり得るため要ユーザー承認、R-7)。
...
- **R-7(SSH バンドルのビルド供給)**: JGit 7.5 の `org.eclipse.jgit.ssh.apache(.agent)` + Apache MINA sshd 推移依存を、jgit core と同じく出荷バンドル(Gradle fat-bundle / Tycho update-site)へ供給・OSGi 解決できるかは PR-3 着手前の事前確認事項(U-10)。既存 p2/Maven ソースで解決不能なら p2 追加が要り、ビルド構成変更に当たるため要ユーザー承認。sshd スタックの OSGi クラスローディング・agent の native/JNA は実機検証。
...
- **U-10**(ビルド供給・PR-3 着手前): JGit 7.5 の SSH バンドル(`org.eclipse.jgit.ssh.apache(.agent)`)+ Apache MINA sshd 推移依存を、jgit core と同じ経路で Gradle fat-bundle と Tycho update-site の双方に供給・解決できるか確認。解決不能時の p2 追加は要ユーザー承認(§21 R-7)。
