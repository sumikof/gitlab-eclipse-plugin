...
- **SSH transport(必須依存の追加・明示)**: JGit core だけでは SSH 通信を開始できない(`SshSessionFactory` 実装が無いと fetch/push が実行時 transport error)。したがって SSH remote を扱うには **`org.eclipse.jgit.ssh.apache`(Apache MINA sshd ベース)を必須依存として追加**する。**プロセス全体の静的置換(`SshSessionFactory.setInstance(...)`)はしない**(同一 JVM の EGit や他プラグインの factory を奪い、認証・proxy・host-key 処理を壊すため)。代わりに、当該 `FetchCommand`/`PushCommand` の **`setTransportConfigCallback` から対象 `SshTransport` にだけ** プラグイン所有の `SshdSessionFactory` を設定し、ライフサイクルもプラグイン内に閉じる。鍵/エージェント/known_hosts はユーザーの `~/.ssh`(および ssh-agent)を用いる。self-managed 環境の host-key 検証(未知ホストの扱い)は実機検証項目(§21 R-3)。
  ...
  - **SSH バンドルの供給(依存の種別で経路を分ける・確定)**:
    - **ホストバンドル `org.eclipse.jgit.ssh.apache`(通常バンドル)**: jgit core と同一の **`eclipseDependencies`→`Require-Bundle`+p2deps** 経路(EGit p2 repo:188 から解決)。feature.xml でも kotlinLibraries fat-jar でもない。
    - **agent `org.eclipse.jgit.ssh.apache.agent`(フラグメント)**: これは **`Fragment-Host: org.eclipse.jgit.ssh.apache` のフラグメント**で、`com.sun.jna` / `com.sun.jna.platform` を要求する。**`Require-Bundle` 経路では供給できない**(build.gradle.kts:231-233 は `eclipseDependencies` を一律 `Require-Bundle` 化するが、フラグメントは Fragment-Host でホストに attach するもので Require-Bundle の対象ではない)。したがって **feature(feature.xml のプラグイン項目)または p2 IU requirement でフラグメントを明示導入**し、ホストに attach させる。**JNA(`com.sun.jna`, `com.sun.jna.platform`)を依存閉包に含める**。
    - **Apache MINA sshd 推移依存(`org.apache.sshd.osgi` 等)**: OSGi バンドルなら Require-Bundle 経路、素 jar しか無いものは `kotlinLibraries` fat-jar リストへ、と依存ごとに振り分ける。
  ...
  - **ssh-agent 認証の依存(JGit 7.5)**: 鍵を `ssh-agent` のみに登録した環境では `org.eclipse.jgit.ssh.apache` 単体では agent connector が無い。**agent フラグメント(上記フラグメント経路)+ JNA** を供給して公開鍵ファイル + agent 双方をカバーする。フラグメント + JNA が feature/p2 経由で 2024-09 の依存閉包から解決・実インストールされるかは U-10 で確認。
...
- **U-10**(ビルド供給・PR-3 着手前): 機序は確定(§7.1: 通常バンドル=Require-Bundle+p2deps、フラグメント=feature/p2 IU、素 jar=kotlinLibraries fat-jar)。残確認: (1) **Gradle** = EGit p2 repo(build.gradle.kts:188)が host bundle・agent フラグメント・`org.apache.sshd.osgi`・JNA を提供するか。(2) **Tycho** = root pom.xml の `2024-09` がそれらの IU(フラグメント・JNA 含む)を含むか(EGit p2 は Tycho には効かない)。(3) **agent フラグメント + JNA の依存閉包が生成 update-site から実インストール**され、ホストに attach するか。**(2)(3) が否なら root pom.xml への p2 追加が必須=ビルド構成変更=要ユーザー承認**(§21 R-7)。SSH 第一案採用(U-8)は Tycho 側 pom.xml の p2 追加を要する可能性が高い点をユーザーに事前共有する。
