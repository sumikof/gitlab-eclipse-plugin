...
→ **A15**: A11 の読み戻し検証器を独立に呼べる形にし、**秘匿フィールドが sentinel を保持していない
組み立て結果**を渡す fixture を置く。検証器がそれを拒否しない実装では必ず失敗させる。

これは §8.1 の A9 と同じ扱いである。production の入口からの検査(A11)を**置き換えず足す**。
...
→ **A14**: production の供給元そのものに専用条件を置く。アンカーはテスト側に直接書いた
リテラルの FQCN とする(§8 の A5 と同じ理由)。

- **(a)** 走査結果が、**秘匿フィールドを持たない既知のクラス**を含む
  (アンカー: `com.gitlab.eclipse.lsp.WebviewInfo` — `id` / `title` / `uris` のいずれも
  名前パターンに当たらない)。固定リスト誤配線ならこれを含まない。
- **(b)** 走査結果が、**より深いサブパッケージの入れ子クラス**を含む
  (アンカー: `com.gitlab.eclipse.lsp.webview.WebviewUriResolver$WebviewResolution$Resolved`)。
  再帰走査が失われていればこれを含まない。
...
| sentinel を保持しない組み立て結果の fixture | A11 の読み戻し検証器自身が固定されること(A15・§8.5) |
| production 供給元のアンカー 2 件 | 供給元の誤配線と再帰走査の欠落が固定されること(A14・§8.6) |
...
| A14 | **production の供給元そのもの**について、走査結果が (a) 秘匿フィールドを持たない既知のクラス(`com.gitlab.eclipse.lsp.WebviewInfo`)と (b) より深いサブパッケージの入れ子クラス(`…WebviewUriResolver$WebviewResolution$Resolved`)を含む。固定リストへの誤配線と再帰走査の欠落をそれぞれ落とす(§8.6) |
| A15 | A11 の読み戻し検証器に、秘匿フィールドが sentinel を保持していない組み立て結果を渡す fixture があり、検証器がそれを拒否しない実装では失敗する(§8.5) |
...
| production の供給元を固定リストに差し替える | A14(a) |
| production の供給元の再帰走査を 1 階層だけにする | A14(b) |
