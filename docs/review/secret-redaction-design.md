...
- **D3(推移閉包)**: **被検出クラスを型に持つフィールドを 1 つでも持つ `data class` は、
  それ自身も被検出クラスとする。** 不動点に達するまで反復する。

**D3 が無いと §7.5 の迂回が別の形で再発する。** D1/D2 はフィールド単位の判定なので、
`data class Envelope(val options: HttpAgentOptions)` のように**直接のフィールド名も型も
D1/D2 に当たらない**保持クラスは検出されない。すると A5 の集合にも A1 の対象にも入らず、
`Envelope.toString()` が `options.certKey` を直接補間しても全条件が通る。

これは「秘匿フィールドは必ずいずれかの被検出クラスに属する」という不変条件
(D1/D2 がフィールド単位であることから成る)が、**直接フィールドについてしか
成立していなかった**ということである。D3 が保持側まで閉じる。

> **実測**: D3 は今日 1 クラスも増やさない。被検出クラスを保持するフィールドは 6 箇所あり、
> うち 2 箇所(`GitLabLanguageServerConfigurationParams.httpAgentOptions` / `.codeCompletion`、
> `EgressConfigSnapshot.proxy`)の保持側は既に D1 で検出済み、残り(`OAuthTokenProvider` /
> `GitLabHttpClient` / `GitLabProxyAuthenticator` / `BypassAwareProxySelector`)は
> すべて非 `data class` で §7.1 により除外される。
> **したがって D3 は純粋に R2(次の違反を止める)のための規則であり、production に発火入力が無い。**
> その識別性は A16(§8.7)で担保する。
...
**この再帰が「外側」に到達できることは D3(§7.2)が保証する。** 被検出クラスを保持する
`data class` は、直接のフィールドが D1/D2 に当たらなくても D3 によって被検出クラスになるため、
外側自身が A5 の集合と A1 の対象に入る。D3 が無いと、保持クラスが検出されないまま
その `toString()` が入れ子の秘匿成分を直接補間できてしまう。
...
### §8.7 D3(推移閉包)に production の発火入力が無い(A16 の理由)

§7.2 の実測のとおり、**D3 は今日 1 クラスも増やさない**。したがって D3 の判定を削除する変異を
当てても、検出集合は変わらず A5 も A1 も落ちない。§8.3 の一般則がそのまま当てはまる。

→ **A16**: **被検出クラスを型に持つが、直接のフィールド名も型も D1/D2 に一致しない `data class`**
を fixture として検査対象に注入し、それが検出集合に入ることを固定する。
D3 を削る変異がこの 1 本だけを落とすこと。

fixture の形は Codex 設計レビュー #58 round 5 が挙げた例をそのまま採る:
`data class Envelope(val options: HttpAgentOptions)`(`options` は D1 に当たらず `HttpAgentOptions` は
`Throwable` でもない)。加えて、その `toString()` が入れ子の秘匿成分を**直接補間する**形にすることで、
検出漏れが実際の漏洩に繋がることまで固定する。

**D3 で検出されたクラスは自分自身の秘匿フィールドを持たない。** その A1 の対象は
§7.5 の再帰で渡した**入れ子の sentinel** である。
...
| 被検出クラスを保持するが直接は D1/D2 に当たらない `data class` の fixture | D3(推移閉包)が固定されること(A16・§8.7) |
...
| A16 | 被検出クラスを型に持つが直接のフィールド名も型も D1/D2 に一致しない `data class` の fixture が検出集合に入る。D3(推移閉包)を削る変異がこの 1 本だけを落とす(§8.7) |
...
| **検出**を無効化する | A5 + A14 + A13(production と fixture が同じトップレベル検出経路を使うため、production の実測集合も空になり A5・A14 が必ず落ちる) |
| **組み立て**を無効化する | A6 + A13(組み立てないことは「検査していない」なので A6 が落ちる) |
| **出力 assert** を無効化する | **A13 のみ**(検出も組み立ても成立するため他は落ちない) |
| D3(推移閉包)の判定を削除する | **A16 のみ**(production に発火入力が無い。§8.7) |
