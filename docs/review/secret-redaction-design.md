...
> **ある条件が十分に固定されているとは、それを空虚にする変異が
> 「テストそのものを削除する」以外に残っていない状態をいう。**

理由: **テストの存在は、テストでは固定できない。** テストを 1 本削除すれば
「テストが 1 本減った」という事実が残るだけで、**それを検査するものは原理的に無い**
(検査を足しても、その検査自体を削除できる)。これはどんなテストスイートも到達する床である。

したがって:

- **production コードの変異**、および **ハーネスの変異でテストが存在したまま盲目になるもの** は、
  すべて閉じなければならない。本レビューの 30 件超はすべてこちらだった。
- **テストの削除**は、テストでは防げない。防ぐのは**差分レビュー**であり、それが正しい階層である。

**運用上の判定**: 各条件について「**全テストが存在したまま、この条件を空虚にする変異はあるか**」を問う。
あるなら未完了。「テストを消す」以外に無いなら完了。

> この停止条件は controller の設計判断である。Codex 設計レビュー #58 の round 9 と round 10 で
> 同じ問いを提示したが回答が得られなかったため、**未回答のまま曖昧にせず、
> 判断として明記する**という扱いにした。誤りであれば、それは
> 「テストの存在をテストで固定できる」という主張になるので、その形で反証できる。
...
| ID | 条件 |
|---|---|
| A1 | **検出されたすべてのクラスの、exemption を除くすべての秘匿フィールド**について、**その秘匿フィールドだけが異なる 2 つのインスタンス**を組み立て、`toString()` の出力が完全に一致する(出力不変性・§7.3)。sentinel の文字列検索は失敗メッセージの診断として併用するが、条件は不変性の側である。**固定件数の条件にしない** — 件数で書くと、将来 D1/D2 に一致するクラスが増えて A5 の期待リストを正しく更新した後も、その新クラスが sentinel 検査を受けず R2 が成立しなくなる。(実測: 現在の対象は 14 クラス。検出 15 から、exemption 済みフィールドしか持たない `CodeCompletion` を除いた数。この 14 は条件ではなく現況である) |
...
**本表の網羅性の基準は §8.8 の停止条件である** — 「全テストが存在したまま、ある条件を
空虚にする変異」がすべて列挙されていること。「テストそのものを削除する」変異は対象外。
