...
| `Throwable` に代入可能 | **同一クラス**で message が異なる 2 つの `RuntimeException` |
...
`data class`)を検査対象に注入し、**検出器が漏洩固有の診断でそれを拒否すること**を固定する。
...
| A13 | 意図的に漏らす control fixture(生成 `toString()` を持つ秘匿名フィールド付き `data class`)を検査対象に注入し、検出器が**漏洩固有の診断で**それを拒否する。**「何らかの理由で拒否された」では足りない** — 組み立てを無効化すると A6 が「検査していない」として拒否するため、拒否の事実だけを見ると A13 が通ってしまう。検出 → 組み立て → 出力 assert の全経路を端から端まで固定する(§8.4) |
