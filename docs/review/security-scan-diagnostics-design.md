...
| `CommandWaiters` | `COMMAND` 起動の応答待ち(正規化パス → 待機 id の FIFO)。可視の応答(F6)を返すためだけに持つ(§9.1.1 / §13.2) |
...
entered = true                                 // 未開始検出用(§9.1.1)
  v (§15.1 の Mutex 下)
ConnectionConfigGeneration の seqlock 下で設定と SECURITY_SCAN_ENABLED を読む
  |- 不安定 -> 中止(COMMAND なら consumeById して通知)
  |- enabled == false -> **送信せず consumeById して終了(通知しない)**(§9.1.1)
  v
server.didChangeConfiguration(読んだ設定)      // 同一コルーチンで先に送る
  |- 例外 -> 中止(COMMAND なら consumeById して通知)
  v
server.runSecurityScan(SecurityScanParams(uri, source))
  |- 例外 -> 中止(COMMAND なら consumeById して通知)
  v
COMMAND なら waiterId に応答期限(60 秒)を登録し markDeadlineArmed(waiterId, epoch)
  |    // ★ 送信を試みた後に起動する(§13.2)

[Job の完了時・§9.1.1]
isDeadlineArmed(waiterId) == false -> onSendFailed(waiterId, ...)   // 万能の後始末
```
...
- **未開始の遷移をログだけで放置しない(Codex round18-P2)。**共有スコープがキャンセル済みなどで**有効化 Job が開始されない**と、直前の無効化で奇数になった `sourceEpoch` が永久に停止したままになる。設定値は既に `true` なので以後の `performOk` は遷移を検出せず、**スキャン送信は再開しても診断だけがすべて破棄され続ける**。
  ...
  したがって **`sourceEpoch` の停止状態は、次の送信要求の時点で現在の設定へ収束させる**。

  **収束は §9.1 の最初のゲート(`SECURITY_SCAN_ENABLED == false` で終了)より前に置く(Codex round20-P2)。**未開始だった遷移が**無効化**だった場合、設定値は既に `false` なので、ゲートより後に収束を置くと**到達しない**。その間 `sourceEpoch` は稼働中のままで遅延診断も受理されうる。当初 (a) の `Mutex` 内に置いていたが、それでは無効化方向の収束が次回起動まで遅れる。

  確定仕様: `runSecurityScan` core の**先頭**(ゲート判定より前)で、現在の `SECURITY_SCAN_ENABLED` と `isSuspended(source)` を突き合わせ、食い違っていれば **`reconcileSource(source, desiredSuspended)`** を呼ぶ。同じ収束処理を `onActivate()`(バンドル起動)でも行う。

  `reconcileSource` は CAS による偶奇合わせだけで、送信も通知も監査も行わない。したがってゲートより前に置いても「設定 OFF のときは何も起きない」という §9.1 の性質は保たれる(`Mutex` も取らない)。
...
**LS 停止時の扱い**: `CommandWaiters.clear(epoch)` を呼び、**除去した待機数のぶんだけ** §11.3 に従って `COMMAND` に通知し(旧接続の応答は §14.6 で破棄されるため、通知しなければ永久に無反応になる)、監査行に `outcome=cancelled` を残す。保持している状態はパスごとの待機 id の列だけなので、これ以外に破棄するものは無い(**採番器は初期化しない**。§9.1.1)。
...
| L8 | 設定遷移の適用コルーチンが開始されなかった場合、`sourceEpoch` の是正が**次のスキャン要求または次回起動まで遅れる**。また **marker の除去は次の掃除(LS 停止時の epoch 選択削除、または次の無効化遷移)まで遅れる** | §9.1.1 (b)。共有 `CoroutineScope` がキャンセル済みのときにのみ起こる。収束処理はゲートより前に置くので有効化・無効化のどちらの方向でも到達する |
...
| K10 | `COMMAND` の通知が保存由来の応答に消費されうる(§9.1.1 の L2) | 低 | 同上。**応答側に相関 ID が無い**ため、どの応答がどの要求のものか区別できない(待機 id は自分が登録した待機を守るためのもので、応答との対応づけには使えない)。コマンドを再実行すれば解消する |
