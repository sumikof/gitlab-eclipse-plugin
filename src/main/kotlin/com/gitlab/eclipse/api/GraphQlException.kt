 ...
 * null); callers use it to distinguish "execution provably never started" (`data` absent) from
 * "cannot be proven either way" (`data` present). This class only carries the flag; it performs no
 * classification itself. [messages] holds the `message` strings extracted from the
