...
/**
 * In-memory read-only [IEncodedStorage] over a [MergedYamlContent].
 *
 * Implements the encoded variant (not plain IStorage) so the editor decodes with UTF-8
 * regardless of the workspace-default charset. [getContents] reads [MergedYamlContent.text]
 * at call time, so a later text update is reflected on the next read.
 */
class MergedYamlStorage(private val content: MergedYamlContent, private val name: String) : IEncodedStorage {
